...
export function registerLiveAgentTurn(
    event: { type?: string; replayed?: boolean; threadId?: string; thread_id?: string; turnId?: string; turn_id?: string },
    ...
    if (key && authoritativeTurns.has(key)) return event.type === "usage.updated";
    if (key) liveTurns.add(key);
    return true;
}

export function reconcileLiveAgentTurns(threadId: string, activeTurnId: string, authoritativeTurns: ReadonlySet<string>, liveTurns: Set<string>) {
    authoritativeTurns.forEach((key) => liveTurns.delete(key));
    const activeKey = threadId && activeTurnId ? `${threadId}\0${activeTurnId}` : "";
    if (activeKey && !authoritativeTurns.has(activeKey)) liveTurns.add(activeKey);
}
