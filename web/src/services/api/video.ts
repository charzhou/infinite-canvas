...
import { getMediaBlob, resolveMediaUrl, storeMediaFile, uploadMediaFile, type UploadedFile } from "@/services/file-storage";
import { imageToDataUrl } from "@/services/image-storage";
import { boolConfig, buildApiUrl, modelOptionName, resolveModelRequestConfig, resolveModelScript, withLocalProxy, type AiConfig, type ModelRequestConfig } from "@/stores/use-config-store";
...
type XaiVideoTask = { request_id?: string; status?: "pending" | "done" | "completed" | "failed" | "expired"; error?: { message?: string } | string | null };
...
export type VideoGenerationTask = { id: string; provider: "openai" | "gemini" | "xai" | "plugin"; model: string; adapter?: "sub2api" };
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] = [], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    if (requestConfig.providerId === "sub2api") return createSub2ApiVideoTask(requestConfig, selectedModel, prompt, references, options);
    if (requestConfig.apiFormat === "xai") return createXaiVideoTask(requestConfig, selectedModel, prompt, references, options);
...
async function pollXaiVideoTask(config: ModelRequestConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        ...
        if (state.status === "failed" || state.status === "expired") return { status: "failed", error: readApiErrorMessage(state.error) || forkVideoText(state.status === "expired" ? "xaiGenerationExpired" : "xaiGenerationFailed") };
        return { status: "pending" };
    } catch (error) {
...
function assertVideoConfig(config: ModelRequestConfig, model: string) {
    ...
    if (!config.baseUrl.trim()) throw new Error(apiText("baseUrlRequired"));
    if (config.authMode !== "oidc" && !config.apiKey.trim()) throw new Error(apiText("apiKeyRequired"));
}
