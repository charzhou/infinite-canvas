...
export default function VideoPage() {
    ...
    const generate = async () => {
        ...
        try {
            ...
            const log = buildLog({ prompt: snapshot.text, model, config: snapshot.config, references: snapshot.references, videoReferences: snapshot.videoReferences, audioReferences: snapshot.audioReferences, durationMs: performance.now() - batchStartedAt, status: "failed", error: errorMessage });
            selectPreviewLog(log);
    ...
    }, [videoCommand, clearVideoCommand, submitting, t, updateAgentTask]);
        ...
        try {
            ...
            for (let attempt = 0; performance.now() < deadline; attempt += 1) {
                ...
                if (state.status === "completed") {
                    ...
                    const completedLog = { ...log, status: "success" as const, durationMs: nextVideo.durationMs, video: nextVideo, error: undefined };
                ...
                await delay(Math.min(videoPollDelay(attempt), remainingMs));
            }
            throw new Error(t("videoWorkbench.timeout"));
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : t("workbench.generationFailed");
            if (agentTaskId) updateAgentTask(agentTaskId, { status: "failed", successCount: 0, failCount: 1, error: errorMessage });
            const failedLog = { ...log, status: "failed" as const, durationMs: Date.now() - log.createdAt, error: errorMessage };
                ...
                <section className="grid gap-3 lg:min-h-0 lg:overflow-hidden xl:grid-cols-[420px_minmax(0,1fr)]">
                    ...
                    <div className="thin-scrollbar rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto lg:p-5">
                        <div className="mb-4 flex items-center justify-between gap-3">
                            ...
                            {running ? <Tag className="m-0 px-2 py-1">{t("workbench.generating")} {activeTaskCount}</Tag> : null}
                        </div>
                        {results.length ? (
