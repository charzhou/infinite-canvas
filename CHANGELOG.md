...
+ [优化] Sub2API 专用渠道缓存当前会话内已上传的本地参考文件，重复生成时直接复用对应 `file_id`，避免重复调用 `/v1/files`。

## v0.18.16 - 2026-09-24

+ [修复] Sub2API 专用渠道上传本地视频或音频参考素材时，对网络错误、408、429 和 5xx 自动重试，避免一次临时失败直接终止视频创建。

## v0.18.15 - 2026-09-24

+ [吸收] 同步上游官方仓库 APIMart 赞助说明与宣传资源。

## v0.18.14 - 2026-09-24

+ [修复] Sub2API Seedance 480p 宽屏视频统一发送标准 `16:9` 宽高比，避免像素取整后变成 `427:240`。

## v0.18.13

+ [调整] 视频创作台为 Sub2API Seedance 与 MiniMax H3 分别限制首尾帧模式最多 2 张、多图参考模式最多 9 张参考图。
+ [吸收] 同步上游画布性能与插件更新：本地 WebP 缩略图、画布拖拽和连线裁剪优化、生图请求超时、悬浮操作按钮及 ZCode 插件支持。

## v0.18.12 - 2026-09-18

+ [调整] Sub2API 接入相关界面统一使用“算力接入”作为用户可见名称，内部渠道标识与协议保持不变。

## v0.18.11 - 2026-09-18

+ [新增] Sub2API 渠道统一为单实例，支持在 OIDC 授权、长期 API Key 和外部授权链接之间验证后切换，并允许配置手动 Key 的模型能力与协议。

## v0.18.10 - 2026-09-17

+ [新增] Sub2API OIDC 模型目录新增 MiniMax H3 视频模型，并复用现有沧元视频协议和参考媒体能力。

## v0.18.9 - 2026-09-11

+ [调整] 所有异步视频任务的前端轮询超时时间统一延长至 60 分钟。

## v0.18.8 - 2026-09-11

+ [调整] 视频创作台参考媒体提示仅保留用户操作说明，移除 Sub2API 网关上传实现细节。

## v0.18.7 - 2026-09-11

+ [新增] Sub2API OIDC 模型目录新增 Seedance 2.5 视频模型，并沿用 Seedance 沧元协议适配。

## v0.18.6 - 2026-09-11

+ [调整] 视频创作台的 Seedance 参考视频与参考音频改用与参考图一致的横向缩略卡片交互，支持拖放添加、预览、排序和悬浮删除。
...
+ [调整] Sub2API 的 Seedance 模型改用沧元视频字段创建任务，按网关任务 ID 查询结果，并在创建前拒绝暂不支持的本地视频或音频参考。

## v0.18.2 - 2026-09-09

+ [调整] Seedance 2.0 系列内置模型 ID 统一改用点号版本格式。

## v0.18.1 - 2026-09-09

+ [调整] xAI 视频模型将参考模式显示为“首帧”与“多图参考”，并按所选模式分别发送首张或全部参考图。
...
+ [新增] Tegical OIDC 授权模型目录新增 `gpt-image-2.5-sunburst` 与 `gpt-image-2.5-flare` 图片模型。

## v0.17.0 - 2026-09-09

+ [新增] 吸收上游本地代理、节点分组、视频生成模式与任务恢复能力。
+ [调整] 图像和视频生成设置改为分辨率、比例及参考素材的组合配置，支持 Gemini 视频调用。
+ [修复] WebDAV 删除同步、远程媒体保存、提示词文本编号、多参考图编辑与 URL 渠道导入问题。

## v0.16.4 - 2026-08-29

+ [修复] 文档站默认英文路径不再因内部语言重写产生重定向循环。
+ [优化] 文档站移动端折叠菜单新增分类切换入口，桌面端增加随滚动高亮的本页目录。
+ [修复] 图片接口远程结果统一下载、校验并本地保存，跨域无法读取时保留可显示的原链接。
...
+ [修复] 视频创作台在视频下载并保存后立即结束生成状态，不再因等待媒体元数据而需要刷新页面查看结果。

## v0.16.1 - 2026-08-18

+ [修复] 恢复视频插件布尔参数配置的公共导出，修复前端构建失败。
## v0.16.0 - 2026-08-18
...
+ [调整] 吸收上游 v0.15.1 的中英文国际化、Agent Skill 与消息元数据、多图交互、本地存储及画布稳定性更新。
...
+ [调整] 吸收上游 v0.13.0 的 Agent 模型选择、MCP 预热、会话历史、诊断日志、流式交互与节点缩放稳定性更新。

## v0.13.0 - 2026-08-03

+ [新增] 画布右侧 Agent 支持按当前账号可用范围选择 Codex 模型与推理强度。
+ [优化] Canvas Agent 升级 Codex 并在启动时输出和检查相关版本。
...
+ [优化] Agent 流式回复取消逐词动画排队和当前消息离屏占位。
+ [优化] Agent 统一通过公共日志输出 Info 以上信息并规范转发日志时间。
+ [修复] MCP 初始化期间禁用 Agent 按钮与回车发送，避免首条消息提前清除服务加载状态。
+ [修复] Agent 首次发送消息时立即保留并展示用户内容，不再晚于思考状态。
...
+ [新增] OIDC 授权模型目录新增 Seedance 2.0 Fast 视频模型。

## v0.12.1 - 2026-07-31

+ [新增] 吸收上游 Canvas Agent v0.12.0 的权限、任务进度、会话与诊断能力。
+ [修复] Canvas Agent 重构后继续兼容旧版无线程标识的实时事件。
...
+ [优化] 视频任务状态轮询改用有上限的指数退避并延长等待时限，减少长时间生成期间的重复请求。

## v0.11.4 - 2026-07-29

+ [优化] 视频创作台支持并发提交多个生成任务，并可从生成记录切换查看各任务状态和结果。

## v0.11.3 - 2026-07-29

+ [修复] OIDC 授权回调会提示模型 scope 未获客户端允许，且不再将 Provider 原始诊断写入地址栏。

## v0.11.2 - 2026-07-29

+ [调整] OIDC 算力渠道改为用户选择静态模型目录后按所选模型发起授权，更换模型需重新授权。
+ [新增] OIDC 静态模型目录新增 `seedance-2-0` 与 `seedance-2-0-mini` 视频模型。

## v0.11.1 - 2026-07-29

+ [修复] xAI 视频生成改用 Sub2API 支持的 `size` 与 `images` 请求字段。
...
+ [修复] OIDC 视频状态轮询遇到上游连接重置时自动重试一次，减少误报网关不可用。
...
+ [修复] xAI 视频时长按 15 秒上限展示并写入生成记录，避免界面与实际请求不一致。
+ [新增] 视频渠道支持 xAI Imagine API，可使用 Grok Imagine 模型生成文本、首帧图或参考图引导的视频。
