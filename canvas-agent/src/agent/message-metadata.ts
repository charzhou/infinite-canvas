...
type MessageMetadataRecord = { version: 1; clientMessageId: string; threadId?: string; turnId?: string; itemId?: string; createdAt: number; metadata: AgentMessageMetadata };
type MetadataMessage = { role: string; threadId: string; turnId: string; itemId: string; clientMessageId?: string };
...
export class MessageMetadataStore {
    ...
    bindThread(clientMessageId: string, threadId: string) {
        return this.run(async () => {
            ...
            if (!record) return;
            const sameThread = record.threadId === threadId;
            await writeJson(targetFile, { ...record, threadId, turnId: sameThread ? record.turnId : undefined, itemId: sameThread ? record.itemId : undefined });
            await removeFile(pendingFile);
        });
    }

    bindTurn(clientMessageId: string, threadId: string, turnId: string, itemId: string) {
        return this.run(async () => {
            if (!clientMessageId || !threadId || !turnId || !itemId) return;
            await this.ensureReady();
            const pendingFile = this.pendingFile(clientMessageId);
            const targetFile = this.threadFile(threadId, clientMessageId);
            const record = await readRecord(targetFile) || await readRecord(pendingFile);
            if (!record) return;
            await writeJson(targetFile, { ...record, threadId, turnId, itemId });
            await removeFile(pendingFile);
        });
    }
    ...
    mergeThread<T extends MetadataMessage>(threadId: string, messages: T[]) {
        return this.run<Array<T & AgentMessageMetadata>>(async () => {
            await this.ensureReady();
            const records = (await readRecords(this.threadDirectory(threadId))).filter((item) => item.threadId === threadId && item.turnId && item.itemId);
            const byScope = new Map(records.map((item) => [messageScopeKey(item.turnId!, item.itemId!), item]));
            return messages.map((message) => {
                if (message.role !== "user" || message.threadId !== threadId) return message;
                const record = byScope.get(messageScopeKey(message.turnId, message.itemId));
...
async function readRecord(file: string): Promise<MessageMetadataRecord | undefined> {
    try {
        ...
        if ((turnId && (!threadId || !itemId)) || (itemId && (!threadId || !turnId))) throw new Error(`Message metadata scope is invalid: ${file}`);
        return { version: STORAGE_VERSION, clientMessageId, createdAt: Number(value.createdAt) || 0, metadata, ...(threadId ? { threadId } : {}), ...(turnId ? { turnId } : {}), ...(itemId ? { itemId } : {}) };
    } catch (error) {
        if (isMissing(error)) return undefined;
        throw error;
    }
...
function messageScopeKey(turnId: string, itemId: string) {
