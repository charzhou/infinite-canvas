...
test("message metadata survives Agent restarts", async (context) => {
    ...
    await fixture.store.bindTurn("message-1", "thread-1", "turn-1", "synthetic:user");

    const [message] = await fixture.reopen().mergeThread("thread-1", [{ role: "user", threadId: "thread-1", turnId: "turn-1", itemId: "synthetic:user", text: "Generate product images" }]);
...
test("deleting a thread only removes its metadata", async (context) => {
    ...
    for (const number of [1, 2]) {
        ...
        await fixture.store.bindTurn(`message-${number}`, `thread-${number}`, `turn-${number}`, "synthetic:user");
    }
    await fixture.store.removeThread("thread-1");

    const first = await fixture.reopen().mergeThread("thread-1", [{ role: "user", threadId: "thread-1", turnId: "turn-1", itemId: "synthetic:user" }]);
    const second = await fixture.reopen().mergeThread("thread-2", [{ role: "user", threadId: "thread-2", turnId: "turn-2", itemId: "synthetic:user" }]);
    assert.equal(first[0].skill, undefined);
    assert.equal(second[0].skill?.name, "skill-2");
});

test("history metadata is matched by thread, turn, and item instead of client message id alone", async (context) => {
    const fixture = await createFixture(context);
    await fixture.store.recordPending("message-1", { skill: { name: "product-grid", path: "D:\\skills\\product-grid\\SKILL.md" } });
    await fixture.store.bindTurn("message-1", "thread-1", "turn-1", "synthetic:user");

    const messages = await fixture.store.mergeThread("thread-1", [
        { role: "user", threadId: "thread-1", turnId: "turn-1", itemId: "other-user", clientMessageId: "message-1" },
        { role: "user", threadId: "thread-1", turnId: "turn-2", itemId: "synthetic:user", clientMessageId: "message-1" },
        { role: "user", threadId: "thread-1", turnId: "turn-1", itemId: "synthetic:user" },
    ]);
    assert.equal(messages[0].skill, undefined);
    assert.equal(messages[1].skill, undefined);
    assert.equal(messages[2].skill?.name, "product-grid");
});
