    ...
    app.post("/agent/codex/turn", codexMutation(async (req, res) => {
        ...
        try {
            ...
            void runCodexTurn(withAttachmentContext(prompt, attachmentRefs), lifecycleEmit, attachments, {
                ...
                onTurn: (actualTurnId) => {
                    turnId = actualTurnId;
                    void messageMetadataStore.bindTurn(messageId, threadId, turnId, "synthetic:user").catch((error) => logger.warn("Failed to bind message metadata to message", { clientMessageId: messageId, threadId, turnId, itemId: "synthetic:user", error }));
