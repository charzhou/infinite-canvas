...
`PUBLIC_ORIGIN` 必须是 HTTPS 的公开站点地址，并在 Provider 注册精确回调地址：`{PUBLIC_ORIGIN}/api/oidc/callback`。`OIDC_SESSION_KEY` 必须是 base64url 编码的 32 字节随机密钥。`OIDC_PROXY_TIMEOUT_MS` 统一为 `600000`（10 分钟），用于受限上游代理请求。修改 scope 列表会使已有浏览器会话重新授权。
