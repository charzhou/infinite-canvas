...
Render 使用仓库根目录的 `render.yaml` 构建 Docker 镜像。OIDC 相关变量在 Render 控制台中以 Secret 填写：`OIDC_ISSUER`、`OIDC_CLIENT_ID`、`OIDC_CLIENT_SECRET`、`OIDC_SESSION_KEY`、`OIDC_PROVIDER_NAME`、`OIDC_REQUESTED_SCOPES` 和 `PUBLIC_ORIGIN`。
